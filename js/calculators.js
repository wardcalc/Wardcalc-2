...
// 🚨 CRASH DETECTOR
window.onerror = function(msg, url, line) { alert("🚨 MATH CRASH:\n" + msg + "\nLine: " + line); };

// Sandbox protection for Translations and Drugs
if (typeof window.t === 'undefined') { window.t = k => k; }
const t = window.t;

function gv(g){ 
    let e = document.querySelector(`[data-g="${g}"].on`); 
    return e ? parseFloat(e.getAttribute('data-v')) : null; 
}
function allSet(gs){ return gs.every(g => gv(g) !== null); }
function alertMsg() { alert(t('alert_msg') || 'Please complete all required fields.'); }

const S = (num, text) => { if (!text || text === 'undefined') return ''; return `<div style="margin-bottom:12px; display:flex; gap:10px;"><span style="color:var(--gold); font-weight:bold;">${num}.</span> <span style="line-height:1.5;">${text}</span></div>`; };
const D = (title, desc) => { if (!title || title === 'undefined') return ''; return `<div style="margin-bottom:15px; padding-bottom:15px; border-bottom:1px solid rgba(255,255,255,0.05);"><strong style="color:var(--white); font-size:15px; display:block; margin-bottom:4px;">${title}</strong><span style="color:var(--w60); font-size:13px; line-height:1.5;">${desc}</span></div>`; };
const Rx = drug => { if (typeof window.RX !== 'undefined' && window.RX && typeof window.RX[drug] === 'function') return window.RX[drug](); return D(drug.replace(/_/g, ' ').toUpperCase(), 'Consult institutional guidelines.'); };
...
function calc_chads2(){
  ...
  const a2 = gv('ch_a2') || 0; const a1 = gv('ch_a1') || 0;
  if (a2 > 0) s += 2; else if (a1 > 0) s += 1;
...
function calc_bmi(){
  ...
  else if(b < 25) showResult(b, 'kg/m²', 'lo', t('r_bmi_norm'), S(1,t('r_bmi_norm_1')), D(t('r_bmi_norm_d') || 'No meds',''));
...
function calc_centor(){
  let s = 0; ['cen1','cen2','cen3','cen4'].forEach(k => { s += (gv(k) || 0); });
  const age = gv('cenage');
...
function calc_ranson(){
  ...
  else if(s <= 4) showResult(s, '/11', 'md', t('r_ranson_md'), S(1,t('r_ranson_md_1'))+S(2,t('r_ranson_md_2'))+S(3,t('r_ranson_md_3')), D(t('r_ranson_md_d') || 'Close observation','')+Rx('nnj'));
...
function calc_psi(){
  ...
  let score = age + sex; ks.forEach(k => { score += (gv(k) || 0); }); 
...
window.calc_gcs = calc_gcs; window.calc_apgar = calc_apgar; window.calc_wellsdvt = calc_wellsdvt; 
window.calc_dvt = calc_dvt; window.calc_wellspe = calc_wellspe; window.calc_pe = calc_pe; 
window.calc_curb65 = calc_curb65; window.calc_chads2 = calc_chads2; window.calc_childpugh = calc_childpugh; 
window.calc_bmi = calc_bmi; window.calc_egfr = calc_egfr; window.calc_mews = calc_mews; 
window.calc_centor = calc_centor; window.calc_nihss = calc_nihss; window.calc_sofa = calc_sofa; 
window.calc_ranson = calc_ranson; window.calc_psi = calc_psi;
