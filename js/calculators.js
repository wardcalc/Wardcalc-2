...
// --- 1. SAFETY FALLBACKS (Prevents silent crashes!) ---
const S = (num, text) => {
...
// --- 2. THE DRUG PROXY (The Magic Fix) ---
// If a drug function is missing from drugs.js, this catches it and prevents the calculate button from breaking.
const safeRX = new Proxy({}, {
    get: function(target, prop) {
        if (typeof RX !== 'undefined' && RX && typeof RX[prop] === 'function') {
            return RX[prop];
        }
...
function calc_chads2(){
  ...
  let s = 0;
  ks.forEach(k => { s += (gv(k) || 0); }); // Uses toggle check
  
  if(s === 0) showResult(s, '/6', 'lo', t('r_chads_lo'), S(1,t('r_chads_lo_1'))+S(2,t('r_chads_lo_2')), safeRX.aspirin_af());
  else if(s === 1) showResult(s, '/6', 'md', t('r_chads_md'), S(1,t('r_chads_md_1'))+S(2,t('r_chads_md_2')), safeRX.apix_af()+safeRX.riva_af());
  else showResult(s, '/6', 'hi', t('r_chads_hi'), S(1,t('r_chads_hi_1'))+S(2,t('r_chads_hi_2'))+S(3,t('r_chads_hi_3')), safeRX.apix_af()+safeRX.riva_af());
}
...
function calc_psi(){
  ...
  let score = age + sex;
  ks.forEach(k => { score += (gv(k) || 0); }); // Uses toggle check
